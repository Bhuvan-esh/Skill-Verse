...
const vertexShader = `
  ...
  void main(){
    ...
    float n1 = snoise(vec3(pos.x*0.6 + uTime*0.2, pos.y*0.6 + uTime*0.15, pos.z*0.6));
    float n2 = snoise(vec3(pos.x*1.4 - uTime*0.25, pos.y*1.4 + uTime*0.2, pos.z*1.4));
    float n3 = snoise(vec3(pos.x*2.8 + uTime*0.4, pos.y*2.8 - uTime*0.3, pos.z*2.8));

    float pulse = sin(uTime*1.2 + pos.y*2.0)*0.5 + 0.5;
    float disp = (n1*0.45 + n2*0.25 + n3*0.15)*(0.5 + 0.5*pulse);
    disp += uSectionT * 0.35 * snoise(pos * 3.0 + uTime * 2.0);
...
const fragmentShader = `
  ...
  void main(){
    ...
    float sectionMix = clamp(uSectionIndex/5.0, 0.0, 1.0);
...
export default function Hero3DCanvas({ activeWindow = 0, mousePos = { x: 0, y: 0 } }: Hero3DCanvasProps) {
    ...
    const init = () => {
      ...
      const material = new THREE.ShaderMaterial({
        uniforms: {
          ...
          uSectionT: { value: 0 },
          uSectionIndex: { value: activeWindow },
  ...
  // Sync activeWindow to shader uniform
  useEffect(() => {
    if (meshRef.current && meshRef.current.material) {
      const mat = meshRef.current.material as THREE.ShaderMaterial;
      if (mat.uniforms && mat.uniforms.uSectionIndex) {
        mat.uniforms.uSectionIndex.value = activeWindow;
      }
    }
  }, [activeWindow]);
  ...
  return (
    <canvas
      ref={canvasRef}
      className="fixed inset-0 pointer-events-none z-0 w-full h-full opacity-90 animate-fadeIn transition-opacity duration-1000"
    />
  );
}
