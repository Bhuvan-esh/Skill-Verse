...
import {
  ...
  Zap, BrainCircuit, Clock, Check, X, ShieldAlert, ChevronRight, HelpCircle,
  UserPlus, Mic, MessageSquare, Volume2
} from 'lucide-react';

interface MentorSoftSkillsJudgeDeckProps {
  user?: any;
  onBack?: () => void;
}

export default function MentorSoftSkillsJudgeDeck({ user, onBack }: MentorSoftSkillsJudgeDeckProps) {
  ...
  const fetchJudgeDeck = async () => {
    try {
      const res = await fetch('/api/soft-skills/judge-deck');
      ...
      console.warn('Failed to load soft skills judge deck data:', err);
    }
  };
    ...
    setJudgeRows((prev) =>
      prev.map((row) => {
        if (row.id === id) {
          ...
          const clarity = Number(updated.speech_clarity) || 0;
          const presence = Number(updated.presence_body_language) || 0;
          const debate = Number(updated.debate_rebuttal) || 0;
          const delivery = Number(updated.structure_delivery) || 0;
          const bonus = Number(updated.bonus_points) || 0;
          updated.total_score = clarity + presence + debate + delivery + bonus;

          if (updated.total_score >= 80) {
            updated.verdict = 'QUALIFIED_TOP_PERFORMER';
          } else if (updated.total_score >= 50) {
  ...
  const handleAddRow = () => {
    const newRow = {
      ...
      event_name: 'Collegiate Keynote & Debate Championship',
      speech_clarity: 22,
      presence_body_language: 22,
      debate_rebuttal: 22,
      structure_delivery: 22,
      bonus_points: 10,
      total_score: 98,
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      setActionMessage({ type: 'error', text: 'Failed to save draft: ' + err.message });
    }
  };
  ...
  const handleSubmitToVisualArchitects = async () => {
    ...
    try {
      ...
      if (data.success) {
        setActionMessage({
          type: 'success',
          text: 'Official Soft Skill Scores & Judge Verdicts successfully transmitted to Visual Architects! Domain 2 Credits updated.',
        });
        fetchJudgeDeck();
      } else {
  ...
  return (
    <div className="space-y-6 font-sans">
      ...
      {/* Header Console */}
      <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-cyan-500/30 bg-gradient-to-r from-cyan-950/40 via-slate-900 to-indigo-950/40 shadow-2xl relative overflow-hidden">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/30 text-cyan-300 text-xs font-mono uppercase tracking-widest">
              <Mic className="w-3.5 h-3.5 text-cyan-400" />
              <span>Mentor Soft Skills Judge Deck & Evaluation Matrix</span>
            </div>
            <h2 className="text-2xl sm:text-3xl font-extrabold text-white font-heading">
              Soft Skills Scoring & Verdict Station
            </h2>
            <p className="text-xs sm:text-sm text-slate-300">
              Interactive Excel-style scoring grid. Add participants, customize columns, score vocal clarity & presence, and submit official verdicts to Visual Architects.
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="px-5 py-2 rounded-xl bg-gradient-to-r from-cyan-600 via-teal-600 to-indigo-600 hover:opacity-90 text-white font-mono font-bold text-xs shadow-lg shadow-cyan-600/30 transition-all flex items-center space-x-2 cursor-pointer disabled:opacity-50"
            >
        ...
        <div className="space-y-4 animate-in fade-in duration-200">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3.5">
            <div className="glass-panel p-4 rounded-2xl border border-cyan-500/30 bg-slate-900/60 text-center">
              <span className="text-[10px] uppercase font-mono text-cyan-300 font-bold block">Total Evaluated</span>
              <p className="text-2xl font-extrabold text-white font-heading">{judgeRows.length}</p>
              <span className="text-[10px] text-cyan-200/80 font-mono">Participants in Matrix</span>
            </div>
            ...
            <div className="glass-panel p-4 rounded-2xl border border-amber-500/30 bg-slate-900/60 text-center">
              <span className="text-[10px] uppercase font-mono text-amber-300 font-bold block">Pending Transmittal</span>
              <p className="text-2xl font-extrabold text-amber-300 font-heading">
                {judgeRows.filter((r) => !r.submitted_to_architects).length}
              </p>
              <span className="text-[10px] text-amber-200/80 font-mono">Draft State</span>
            </div>

            <div className="glass-panel p-4 rounded-2xl border border-purple-500/30 bg-slate-900/60 text-center">
              <span className="text-[10px] uppercase font-mono text-purple-300 font-bold block">Avg Total Score</span>
              <p className="text-2xl font-extrabold text-purple-300 font-heading">{summary.avgScore}</p>
              <span className="text-[10px] text-purple-200/80 font-mono">Benchmark Metric</span>
            </div>
          </div>
          ...
          <div className="glass-panel p-4 rounded-2xl border border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="relative w-full sm:w-80">
              ...
              <input
                ...
                className="w-full bg-slate-900 border border-white/10 rounded-xl pl-9 pr-3 py-1.5 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500 font-mono"
              />
            </div>

            <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto justify-end">
              {isAddingColumn ? (
                <div className="flex items-center space-x-1.5 bg-slate-900 p-1 rounded-xl border border-cyan-500/40">
                  <input
                    type="text"
                  ...
                  <button
                    ...
                    className="p-1 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white cursor-pointer"
                  >
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl bg-cyan-600/20 hover:bg-cyan-600/30 text-cyan-300 border border-cyan-500/30 text-xs font-mono font-bold transition-all flex items-center space-x-1 cursor-pointer"
                >
              ...
              <div className="p-12 text-center space-y-4">
                ...
                <div className="space-y-1">
                  <h4 className="text-base font-bold text-white font-heading">
                    Spreadsheet Ready For Soft Skill Evaluation
                  </h4>
                ...
                <button
                  onClick={handleAddRow}
                  className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-cyan-600 to-teal-600 hover:opacity-90 text-white text-xs font-mono font-bold shadow-lg transition-all inline-flex items-center space-x-2 cursor-pointer"
                >
                ...
                <table className="w-full text-left text-xs font-mono border-collapse">
                  <thead className="bg-slate-900 sticky top-0 z-10 border-b border-white/10 shadow-md">
                    <tr className="text-slate-400 uppercase text-[10px]">
                      ...
                      <th className="py-3 px-2 w-24 text-center border-r border-white/5">Speech (/25)</th>
                      <th className="py-3 px-2 w-24 text-center border-r border-white/5">Presence (/25)</th>
                      <th className="py-3 px-2 w-24 text-center border-r border-white/5">Debate (/25)</th>
                      <th className="py-3 px-2 w-24 text-center border-r border-white/5">Delivery (/25)</th>
                      <th className="py-3 px-2 w-20 text-center border-r border-white/5">Bonus</th>
                      <th className="py-3 px-3 min-w-[100px] text-center border-r border-white/5 bg-cyan-950/30 text-cyan-200 font-bold">
                        Total Score
                      </th>
                  ...
                  <tbody className="divide-y divide-white/5">
                    {filteredRows.map((row, idx) => {
                      const isTop = row.total_score >= 80;
                      const isFail = row.total_score < 50;
                        ...
                        <tr key={row.id} className="hover:bg-white/[0.03] transition-colors">
                          ...
                          <td className="py-2.5 px-3 border-r border-white/5">
                            <input
                              ...
                              className="w-full bg-transparent border-none text-white font-bold focus:outline-none focus:bg-cyan-950/30 px-1 py-0.5 rounded"
                            />
                          </td>
                          ...
                          <td className="py-2.5 px-3 border-r border-white/5">
                            <input
                              ...
                              className="w-full bg-transparent border-none text-cyan-300 focus:outline-none focus:bg-cyan-950/30 px-1 py-0.5 rounded"
                            />
                          </td>
                          ...
                          <td className="py-2.5 px-3 border-r border-white/5">
                            <input
                              ...
                              className="w-full bg-transparent border-none text-slate-300 text-[11px] focus:outline-none focus:bg-cyan-950/30 px-1 py-0.5 rounded"
                            />
                          </td>
                          ...
                          <td className="py-2.5 px-2 text-center border-r border-white/5">
                            <input
                              type="number"
                              max="25"
                              value={row.speech_clarity}
                              onChange={(e) => handleCellChange(row.id, 'speech_clarity', Number(e.target.value))}
                              className="w-full text-center bg-transparent border-none text-slate-200 focus:outline-none focus:bg-cyan-950/30 px-1 py-0.5 rounded"
                            />
                          </td>
                          ...
                          <td className="py-2.5 px-2 text-center border-r border-white/5">
                            <input
                              type="number"
                              max="25"
                              value={row.presence_body_language}
                              onChange={(e) => handleCellChange(row.id, 'presence_body_language', Number(e.target.value))}
                              className="w-full text-center bg-transparent border-none text-slate-200 focus:outline-none focus:bg-cyan-950/30 px-1 py-0.5 rounded"
                            />
                          </td>
                          ...
                          <td className="py-2.5 px-2 text-center border-r border-white/5">
                            <input
                              type="number"
                              max="25"
                              value={row.debate_rebuttal}
                              onChange={(e) => handleCellChange(row.id, 'debate_rebuttal', Number(e.target.value))}
                              className="w-full text-center bg-transparent border-none text-slate-200 focus:outline-none focus:bg-cyan-950/30 px-1 py-0.5 rounded"
                            />
                          </td>
                          ...
                          <td className="py-2.5 px-2 text-center border-r border-white/5">
                            <input
                              type="number"
                              max="25"
                              value={row.structure_delivery}
                              onChange={(e) => handleCellChange(row.id, 'structure_delivery', Number(e.target.value))}
                              className="w-full text-center bg-transparent border-none text-slate-200 focus:outline-none focus:bg-cyan-950/30 px-1 py-0.5 rounded"
                            />
                          </td>
                          ...
                          <td className="py-2.5 px-2 text-center border-r border-white/5">
                            <input
                              ...
                              className="w-full text-center bg-transparent border-none text-amber-300 font-bold focus:outline-none focus:bg-cyan-950/30 px-1 py-0.5 rounded"
                            />
                          </td>

                          {/* Total Score */}
                          <td className="py-2.5 px-3 text-center border-r border-white/5 bg-cyan-950/20 font-extrabold text-amber-400">
                            {row.total_score} Pts
                          </td>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
            <div>
              <h3 className="text-xl font-bold text-white font-heading flex items-center space-x-2">
                ...
                <span>Soft Skills & Keynote · Top Performers Leaderboard</span>
              </h3>
              <p className="text-xs text-slate-400 font-sans mt-0.5">
                Participants scoring &gt;80 Total Points across vocal clarity, body language, and debate rebuttal.
              </p>
            </div>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-[10px] font-mono font-bold">
                        {p.total_score} Total Pts
                      </span>
                    </div>

                    <div>
                      <h4 className="text-base font-bold text-white font-heading">{p.student_name || 'Participant'}</h4>
                      <p className="text-xs text-cyan-300 font-mono">{p.usn} • {p.department}</p>
                    </div>
                    ...
                    <div className="p-3 rounded-xl bg-black/40 border border-white/5 text-xs font-mono space-y-1">
                      <div className="flex justify-between">
                        ...
                        <span className="text-cyan-300 font-bold">{p.speech_clarity}/25</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400">Body Presence:</span>
                        <span className="text-emerald-400 font-bold">{p.presence_body_language}/25</span>
                      </div>
                    </div>
                  </div>
                ))}
                ...
                <table className="w-full text-left text-xs font-mono">
                  <thead>
                    <tr className="border-b border-white/10 text-slate-400 uppercase text-[10px]">
                      ...
                      <th className="py-3 px-3">Debate</th>
                      <th className="py-3 px-3">Delivery</th>
                      <th className="py-3 px-3">Verdict</th>
                      <th className="py-3 px-3 text-right">Domain 2 Pts</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5">
                    {topPerformers.map((p: any, idx: number) => (
                      <tr key={p.id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="py-3.5 px-3 font-bold text-cyan-300">{p.speech_clarity}/25</td>
                        <td className="py-3.5 px-3 font-bold text-emerald-300">{p.presence_body_language}/25</td>
                        <td className="py-3.5 px-3 text-slate-300">{p.debate_rebuttal}/25</td>
                        <td className="py-3.5 px-3 text-slate-300">{p.structure_delivery}/25</td>
                        <td className="py-3.5 px-3">
      ...
      {activeTab === 'weak_performers' && (
        <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-rose-500/30 bg-slate-950/60 shadow-2xl space-y-6 animate-in fade-in duration-200">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-4">
            <div>
              <h3 className="text-xl font-bold text-white font-heading flex items-center space-x-2">
                ...
                <span>Soft Skills Deficiency Diagnostic & Remediation Station</span>
              </h3>
              <p className="text-xs text-slate-400 font-sans mt-0.5">
                Participants scoring &lt;80 Total Points across vocal modulation, filler words, or nervous body cues.
              </p>
            </div>
          ...
          {weakPerformers.length === 0 ? (
            <div className="p-8 text-center space-y-2">
              <p className="text-xs text-slate-400 font-mono">
                No weak or deficient performers found. All evaluated participants scored &gt;80 points.
              </p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="p-3 rounded-xl bg-slate-900 border border-rose-500/20 text-xs font-mono text-amber-200 space-y-1">
                    <span className="text-amber-400 font-bold uppercase text-[10px] flex items-center space-x-1">
                      ...
                      <span>Actionable Vocal & Presentation Remediation:</span>
                    </span>
