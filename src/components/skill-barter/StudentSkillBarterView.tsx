...
import { Users, Plus, XCircle, User, BookOpen, Layers, GraduationCap, CheckCircle2, ShieldAlert, Lock } from 'lucide-react';
...
export default function StudentSkillBarterView({ user, onRefresh }: SkilloraProps) {
  const isParticipant = !user?.role || user?.role?.toUpperCase() === 'STUDENT' || user?.role?.toUpperCase() === 'PARTICIPANT';

  // Sample Participant Session Requests
  ...
  const handlePostRequest = (e: React.FormEvent) => {
    ...
    if (!isParticipant) {
      setErr('Only registered Participants can publish session requests.');
      return;
    }
    ...
    setMsg('Participant Session Request published successfully!');
    setTopic('');
    setMessage('');
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div className="glass-panel p-6 rounded-2xl border border-indigo-500/20 bg-gradient-to-r from-indigo-950/30 via-slate-900 to-slate-900 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center space-x-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-xs font-semibold mb-2">
            ...
            <span>Participant Peer Hub</span>
          </div>
          <h2 className="text-2xl font-bold text-white font-heading">Skillora — Participant Session Requests</h2>
          <p className="text-sm text-slate-400">
            Browse participant peer requests, view domain topics, and request or offer mentoring assistance.
          </p>
        </div>
        ...
        {isParticipant ? (
          ...
          <div className="flex items-center space-x-2 px-4 py-2.5 rounded-xl bg-slate-900/80 border border-amber-500/30 text-amber-400 text-xs font-semibold">
            <Lock className="w-4 h-4" />
            <span>Participant Only Feature</span>
          </div>
        )}
      </div>

      {!isParticipant && (
        <div className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-xs flex items-center space-x-2.5">
          <ShieldAlert className="w-4 h-4 flex-shrink-0 text-amber-400" />
          <span>You are viewing the Participant Feed. Publishing new session requests is exclusive to logged-in Participants.</span>
        </div>
      )}
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-white font-heading flex items-center space-x-2">
          ...
          <span>Participant Post Session Requests</span>
        </h3>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="w-full max-w-lg glass-panel p-6 rounded-2xl border border-white/10 space-y-5 shadow-2xl">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <h3 className="text-lg font-bold text-white font-heading flex items-center space-x-2">
                <BookOpen className="w-5 h-5 text-indigo-400" />
                <span>Participant Request Details</span>
              </h3>
            ...
            <div className="space-y-3.5 text-sm">
              <div className="flex items-start space-x-3 p-3 rounded-xl bg-slate-900 border border-white/5">
                ...
                <div>
                  <span className="text-xs font-semibold text-slate-400 block">Participant Name</span>
            ...
            <button
              onClick={() => {
                setMsg(`Offered mentoring assistance to ${selectedRequest.name} for topic "${selectedRequest.topic}"!`);
      ...
      {showPostModal && isParticipant && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
          <div className="w-full max-w-lg glass-panel p-6 rounded-2xl border border-white/10 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              <h3 className="text-lg font-bold text-white font-heading">Post Participant Session Request</h3>
            ...
            <form onSubmit={handlePostRequest} className="space-y-4 text-xs">
              ...
              >
                Publish Participant Session Request
              </button>
            </form>
          </div>
        </div>
